Code Examples
=============

Complementing the documentation here, there is a library of Jupyter notebooks
(`also available directly here <https://github.com/batfish/pybatfish/tree/master/jupyter_notebooks>`_)
that show you how to use Batfish to analyze your network.

Basics
++++++
.. toctree::
    :maxdepth: 1

    notebooks/linked/getting-started-with-batfish.ipynb
    notebooks/linked/validating-configuration-settings.ipynb

ACLs and firewalls
++++++++++++++++++

.. toctree::
    :maxdepth: 1

    notebooks/linked/analyzing-acls-and-firewall-rules.ipynb
    notebooks/linked/provably-safe-acl-and-firewall-changes.ipynb
    notebooks/linked/safely-refactoring-acls-and-firewall-rules.ipynb

Routing analysis
++++++++++++++++

.. toctree::
    :maxdepth: 1

    notebooks/linked/introduction-to-route-analysis.ipynb
    notebooks/linked/introduction-to-bgp-analysis.ipynb
    notebooks/linked/analyzing-routing-policies.ipynb

Forwarding analysis
+++++++++++++++++++

.. toctree::
    :maxdepth: 1

    notebooks/linked/introduction-to-forwarding-analysis.ipynb
    notebooks/linked/introduction-to-forwarding-change-validation.ipynb

Failure-impact analysis
+++++++++++++++++++++++

.. toctree::
    :maxdepth: 1

    notebooks/linked/analyzing-the-impact-of-failures-and-letting-loose-a-chaos-monkey.ipynb

Cloud networking
++++++++++++++++

.. toctree::
    :maxdepth: 1

    notebooks/linked/analyzing-public-and-hybrid-cloud-networks.ipynb
