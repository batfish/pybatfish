...
def test_specifiers_up_to_date():
    original = requests.get("https://raw.githubusercontent.com/batfish/batfish/master/questions/Parameters.md").content
    ...
    if outfile.exists():
        os.remove(outfile)
    if hashlib.sha256(original).hexdigest() != hashlib.sha256(checked_in.read_bytes()).hexdigest():
