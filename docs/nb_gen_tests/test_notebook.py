...
def rendered_nb(request: Any, categories: Mapping[str, Any], session: Session, snapshots: None) -> RenderedNotebook:
    """
    ...
    if nb_info.generated:
        ...
        category_matches = [c for c in categories["categories"] if c["name"] == nb_info.path.stem]
        assert len(category_matches) == 1
        category = category_matches[0]
        nb_to_execute = generate_notebook(category, get_name_to_qclass(session), session)
    else:
    ...
    return RenderedNotebook(path=nb_info.path, original=original_nb, rendered=nb_to_execute)
...
def test_notebook_code_output(rendered_nb: RenderedNotebook):
    ...
    try:
        ...
        with open(f"{rendered_nb.path}.testout", "w", encoding="utf-8") as f:
            nbformat.write(rendered_nb.rendered, f)
            pytest.fail(
                f"{rendered_nb.path} failed output validation:\n{e}",
                pytrace=False,
            )
