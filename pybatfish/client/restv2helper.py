...
def add_issue_config(session, issue_config):
    ...
    url_tail = (
        f"/{CoordConstsV2.RSC_NETWORKS}/{session.network}/{CoordConstsV2.RSC_SETTINGS}/{CoordConstsV2.RSC_ISSUES}"
    )
...
def delete_issue_config(session, major, minor):
    ...
    url_tail = f"/{CoordConstsV2.RSC_NETWORKS}/{session.network}/{CoordConstsV2.RSC_SETTINGS}/{CoordConstsV2.RSC_ISSUES}/{major}/{minor}"
...
def get_work_log(session, snapshot, work_id):
    # type: (Session, str|None, str) -> Text
...
def _put(session, url_tail, params=None, json=None, stream=None):
    # type: (Session, str, Optional[Dict[str, Any]], Any|None, Optional[Any]) -> None
